...
class TestBeanieClient:
    ...
    @pytest.mark.asyncio
    async def test_init_creates_connection(
    ...
    @pytest.mark.asyncio
    async def test_get_db_default_returns_database(
        self, document_models: list[type[Document]]
    ) -> None:
        ...
        assert db.name == AppSettings.MONGO.db

    @pytest.mark.asyncio
    async def test_init_force_recreates_connection(
        self, document_models: list[type[Document]]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_init_reuses_existing_connection(
        self, document_models: list[type[Document]]
    ) -> None:
